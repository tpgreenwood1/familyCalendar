...
import { listChoreOccurrencesForWeek, type ChoreOccurrenceDTO } from "@/lib/chores";
import ChoreWeekOverview from "@/components/ChoreWeekOverview";
...
async function getWeekData(ctx: FamilyContext): Promise<{
  ...
  try {
    const { weekStart, occurrences } = await listChoreOccurrencesForWeek(ctx);
...
export default async function ChoreWeekPage({
  params,
}: {
    ...
    <div className="flex min-h-screen flex-col bg-gray-950">
      ...
      <main className="flex flex-1 flex-col items-center px-6 py-16">
        ...
        <div className="mt-10 w-full max-w-6xl px-4">
          <ChoreWeekOverview member={member} weekStart={weekStart} initialOccurrences={occurrences} />
        </div>
      </main>
      <Footer />
    </div>
